...
if (isUser) {
  ...
  if (files.length > 30) {
    warn(
      "Please avoid big changes all at once unless it's absolutely necessary!"
    );
  }

  const projectsChanges = files.filter((f) => f.includes("projects"));
  if (projectsChanges.length) {
    const desktopChangesAndNoTests =
      projectsChanges.filter(
    ...
    const apiChangesAndNoTests =
      projectsChanges.filter(
        (f) =>
          !f.includes("intermediary-") &&
          f.includes("api") &&
          f.includes("api/__tests__")
      ).length === 0;

...
describe('Onboarding', () => {
	it('Cannot visit the page without token', () => {
		cy.visit('/onboarding').url().should('not.include', '/onboarding')
	})
	...
	it('Can visit the page with a token', () => {
		...
		cy.visit('/onboarding').url().should('include', '/onboarding')
	})
	...
	it('Add a description', () => {
		...
		descriptionInput.type('test').type('{enter}').type('test').should('have.value', 'test\ntest')
	})

	it('Add an avatar', () => {
		// Opening modal
		cy.get('[data-testid=avatarOverlay]').click({ force: true })
		...
		fileInput.attach_file('files/image.png', 'image/png').trigger('change', { force: true })
	...
	it('Change the avatar correctly', () => {
		...
		avatar.invoke('val').should('not.eq', avatarValue)
	})
	...
	it('Reset the avatar editor flow', () => {
		...
		cy.get('[data-testid=avatarCropperDialog] .MuiGrid-root:nth-child(2) button').click()

...
export const authController = {
	...
	login: async (data: IUser): Promise<object | BoomType> =>
		_authenticate(data.email, data.password, data.remember),
	...
	onboarding: async (user: RequestUser, data: IUser): Promise<object | BoomType> => {
		...
		const pastLimit: DateTime = DateTime.fromFormat('01/01/1910', 'dd/LL/yyyy')
		...
		try {
			...
			await getManager().transaction(
				async (transactionalEntityManager: EntityManager): Promise<void> => {
					...
					user = await transactionalEntityManager.findOne(User, { email: email })
				}
			)

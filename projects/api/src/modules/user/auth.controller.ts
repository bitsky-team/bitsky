...
export class AuthController {
   ...
   */
  @Post('/signup')
  async signup(@Body() data: UserSignUpData): Promise<object | BoomType> {
    // Hashing password
    const saltRounds: number = parseInt(process.env.SALT_ROUNDS ?? '10', 10);
    ...
    try {
      await getManager().transaction(
        async (transactionalEntityManager: EntityManager): Promise<User> => {
          // Check unique email
          const emailTaken: number = await transactionalEntityManager.count(
            User,
   ...
   */
  @Post('/login')
  async login(@Body() data: UserLogInData): Promise<object | BoomType> {
  ...
  async onboarding(
    ...
    const currentUser: UserFromRequest = request.user;
    const { username, birthdate }: UserOnboardingData = data;
    const { email } = currentUser;

    const convertedBirthdate: DateTime = DateTime.fromFormat(
      birthdate!,
      'dd/LL/yyyy',
    );
    ...
    try {
      ...
      await getManager().transaction(
        async (transactionalEntityManager: EntityManager): Promise<void> => {
          ...
          const usernameTaken: number = await transactionalEntityManager.count(
            User,
            { where: { username } },
          );
